...
## 🌍 Real World Case
Bayangkan Anda bekerja di **bursa saham** dan ingin tahu: "Kapan harga saham berikutnya akan lebih tinggi dari hari ini?" Untuk setiap hari trading, Anda perlu mencari hari berikutnya di mana harga lebih tinggi. Dengan Monotonic Stack, Anda bisa melakukan ini dalam **O(n) time** instead of O(n²)!

Contoh lain: **Website analytics** - mencari "next page view dengan durasi lebih lama" untuk setiap session.

## 📊 Visualisasi Konsep
```
Stock Prices: [73, 74, 75, 71, 69, 72, 76, 73]
Goal: Untuk setiap hari, cari berapa hari lagi sampai harga lebih tinggi?
...
Stack menyimpan INDEX dari elemen yang menunggu next greater element
...
def daily_temperatures(temperatures: List[int]) -> List[int]:
    """
    Untuk setiap hari, hitung berapa hari lagi sampai suhu lebih hangat
    
    Visualisasi proses:
    temperatures = [73, 74, 75, 71, 69, 72, 76, 73]
    
    Monotonic Stack (decreasing):
    Stack menyimpan index dari temperatures yang menunggu
    
    i=0, temp=73:
      Stack kosong, push index 0
      Stack: [0] (berisi index dengan temp 73)
    ...
    Stack masih ada isi? Mereka tidak punya next greater → answer = 0
    ...
    n = len(temperatures)
    answer = [0] * n
    stack = []  # Stack menyimpan INDEX
    
    for i in range(n):
        # Selama current temp lebih besar dari temp di top of stack
...
def next_greater_element(nums: List[int]) -> List[int]:
    """
    Untuk setiap element, cari next greater element di sebelah kanan
...
if __name__ == "__main__":
    ...
    print("\nProses Monotonic Stack Step-by-Step:\n")
    ...
    print("""
Scenario: Trader ingin tahu kapan harga saham naik
Prices: [100, 101, 102, 98, 97, 99, 105, 103]

Pertanyaan: "Berapa hari lagi harga akan lebih tinggi dari hari ini?"
...
Monotonic Stack membantu solve ini dalam O(n) time!
Tanpa stack: O(n²) - untuk setiap hari, scan semua hari berikutnya
    """)
...
1. 📚 Stack menyimpan INDICES (bukan values)
2. 📉 Maintain stack dalam urutan tertentu (increasing/decreasing)
3. 🔍 Ketika ketemu element yang "break" monotonic property:
   → Pop elements dan process them
   → Set their answer
4. ➕ Push current index ke stack
...
Time: O(n) - Setiap element di-push dan di-pop max 1x
...
- ✅ Mencari **next greater/smaller element**
- ✅ Problem dengan kata kunci: "next larger", "next warmer", "stock span"
- ✅ Ketika perlu track elements sambil maintain urutan tertentu
- ✅ Optimasi dari O(n²) nested loop menjadi O(n)

## ⏱️ Time & Space Complexity

- **Time**: O(n) - Setiap element pushed dan popped maksimal 1 kali
- **Space**: O(n) - Stack dapat berisi semua elements dalam worst case
- **Keunggulan**: Jauh lebih cepat dari brute force O(n²)
