...
## 🌍 Real World Case
Bayangkan Anda bekerja di **Amazon warehouse** dengan jutaan produk tersusun berdasarkan ID (sorted). Ketika customer order produk dengan ID tertentu, Anda perlu mencarinya dengan **sangat cepat**. Binary Search memungkinkan Anda menemukan produk dalam **O(log n)** time - jika ada 1 juta produk, hanya butuh maksimal **20 langkah** saja!

Contoh lain: **Dictionary app** mencari kata dalam urutan alfabetis, atau **version control system** mencari commit yang menyebabkan bug (git bisect).

## 📊 Visualisasi Konsep
```
Problem: Cari angka 7 dalam sorted array [1, 3, 5, 7, 9, 11, 13, 15]
...
def binary_search(arr: List[int], target: int) -> int:
    """
    Standard binary search - cari index dari target
...
def search_rotated_array(arr: List[int], target: int) -> int:
    """
    Binary search pada rotated sorted array
...
def find_peak_element(arr: List[int]) -> int:
    """
    Cari peak element (element yang lebih besar dari neighbors)
    
    Visualisasi:
    arr = [1, 2, 3, 1]
    
    Peak adalah element yang arr[i] > arr[i-1] dan arr[i] > arr[i+1]
    Index 2 (value 3) adalah peak
...
- ✅ Array **sorted** atau memiliki sorted property
- ✅ Kata kunci: "search", "find first", "find last", "peak"
- ✅ Butuh **O(log n)** time complexity
- ✅ Bisa **eliminate half** of search space di setiap step

## ⏱️ Time & Space Complexity

- **Time**: O(log n) - Eliminate half each iteration
- **Space**: O(1) - Only use constant pointers
- **Keunggulan**: Exponentially faster than linear O(n) for large datasets
